...
/// Provides a mechanism for consumers to specify the delay between retries
/// given the number of times that a request has already been performed.
typedef NSTimeInterval (^AUTRetryInterval)(NSUInteger retryCount);
...
extern AUTRetryInterval AUTRetryBackOffInterval;
...
/// Invokes aut_retryForReachability:isErrorRecoverable:retryInterval: with a
/// AUTRetryBackOffInterval for the retryInterval block.
...
///
/// @param retryInterval A block that returns the interval between retries.
///
...
- (instancetype)aut_retryForReachability:(RACSignal *)reachability isErrorRecoverable:(AUTIsErrorRecoverableBlock)isErrorRecoverable retryInterval:(AUTRetryInterval)retryInterval;

/// Invokes aut_retryForReachability: with the reachability of the provided
/// host.
- (instancetype)aut_retryForHost:(NSString *)host;

/// Invokes aut_retryForReachability:isErrorRecoverable:retryInterval: with the reachability
/// of the provided host and the provided isErrorRecoverable block and the
/// default AUTRetryBackOffInterval retryInterval.
...
/// of the provided host and the provided isErrorRecoverable and retryInterval
/// blocks.
- (instancetype)aut_retryForHost:(NSString *)host isErrorRecoverable:(AUTIsErrorRecoverableBlock)isErrorRecoverable retryInterval:(AUTRetryInterval)retryInterval;
