...
@interface ALAuthorization : NSObject

@property (nonatomic, readonly) BOOL exists;

typedef void (^ALAuthorizationOnStatus)(ALAuthStatus inAuthStatus, NSError *inError);
typedef void (^ALAuthorizationOnAuthorized)(ALAutomaticAdapter *inAdapter, NSString *inClientID, ALAuthorizationOnStatus inStatusCallback);
typedef void (^ALAuthorizationOnWaitVerdict)(ALAuthStatus inAuthStatus, ALAuthorization *inAuthorization, NSError *inError);

+ (instancetype)authorizationForAdapter:(ALAutomaticAdapter *)inAdapter;

+ (BOOL)installAuthorizationFromURL:(NSURL *)inAccountAuthRedirectURL
                         withSecret:(NSString *)inClientSecret
                           onStatus:(ALAuthorizationOnStatus)inOnStatus
                       onAuthorized:(ALAuthorizationOnAuthorized)inOnAuthorized;

+ (BOOL)waitForAuthorizationForAdapter:(ALAutomaticAdapter *)inAdapter
                             onVerdict:(ALAuthorizationOnWaitVerdict)inOnVerdict;

- (BOOL)askForAuthorizationForClient:(NSString *)inAutomaticClientID;

- (BOOL)installAuthorizationWithOAuthToken:(NSString *)inToken
